...
!-------------------------------------------------------------------------------------------------------------------------------
    ...
    !-------------------------------------------------------------------------------------------------------------------------------
    REAL(DbKi) FUNCTION PIControllerWithLPFilter(error, kp, ki, minValue, maxValue, DT, I0, piP, reset, inst)
        USE ROSCO_Types, ONLY : piParams
    ...
    ! PI controller, with output saturation
        ...
        INTEGER(IntKi)                      :: i                                            ! Counter for making arrays
        REAL(DbKi)                         :: PTerm                                        ! Proportional term
        REAL(DbKi)                         :: RC, alpha       ! Low-pass filter coefficients

        ! Initialize persistent variables/arrays, and set inital condition for integrator term
        IF (reset) THEN
            piP%ITerm(inst) = I0
            piP%ITermLast(inst) = I0
            PIControllerWithLPFilter = I0
        ELSE
            ! Calculate low-pass filter coefficients
            RC = 1.0 / (2.0 * PI * piP%cutoff_frequency)
            alpha = DT / (RC + DT)

            ! Compute PI controller terms
            PTerm = kp*error
            piP%ITerm(inst) = piP%ITerm(inst) + DT*ki*error
            piP%ITerm(inst) = saturate(piP%ITerm(inst), minValue, maxValue)

            ! Apply low-pass filter to the overall output
            PIControllerWithLPFilter = alpha * (PTerm + piP%ITerm(inst)) + (1.0 - alpha) * piP%ITermLast(inst)

            ! Perform saturation on the final output
            PIControllerWithLPFilter = saturate(PIControllerWithLPFilter, minValue, maxValue)
            
            piP%ITermLast(inst) = piP%ITerm(inst)
        END IF
        inst = inst + 1
        
    END FUNCTION PIControllerWithLPFilter
